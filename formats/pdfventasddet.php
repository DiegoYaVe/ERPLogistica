...
var $aligns; 
  ...
  $this->Cell(35);
  $this->Cell(110,32,utf8_decode('REPORTE GENERAL DE VENTAS DETALLADO'),0,0,"C");
  $this->Ln();
...
function Row($data,$bandera)
{
  ...
  for($i=0;$i<count($data);$i++)
    $nb=max($nb,$this->NbLines(191,$data[$i]));
  $h=5*$nb;
  ...
  for($i=0;$i<count($data);$i++)
  {
    ...
    $x=$this->GetX();
    $y=$this->GetY();
    $this->SetFillColor(207,227,255);
      //dibuja la tabla
    $this->Rect($x,$y,191,$h,'FD');
    //solo imprime el texto
    $this->MultiCell(191,5,$data[$i],0,'L');
...
function paquete($id)
{
  ...
  $sqlinf = 'SELECT rc_articulo, rc_modelo, COUNT(*) cantidad, a_nmb FROM remisionesc INNER JOIN articulos ON a_id =  rc_articulo
  WHERE rc_remision = "'.$id.'" AND rc_ligado IS NULL GROUP BY rc_articulo, rc_modelo '; 
  $resultinf = setq($sqlinf);
  while($rowinf = $resultinf -> fetch_array()){
    $nmbinf = $rowinf['a_nmb'];
    if($rowinf['rc_modelo']){
      $nmbinf .= ' '.busca($rowinf['rc_modelo'], 'articulos_variantes', 'av_articulo = "'.$rowinf['rc_articulo'].'" AND av_modelo', 'av_nmb');
    }
    $s.= ' - '.$rowinf['cantidad'].' '.utf8_decode($nmbinf)."\n"; 
  } 
...
while($row = $resultrem -> fetch_array()){
  ...
  $pdf->Row(array($pdf->paquete($row['r_id'])),false);
...
$pdf->RowMin(array(':','','','', 'Totales porcentajes:', '100%', number_format($pordesc, 2).'%', number_format($porfinal, 2).'%', ''),false);
