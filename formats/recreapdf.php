...
$id = $_GET['id'];
$facturas = $_GET['id'];
$emp = busca($_GET['id'],'facturas','f_id','f_empresa');
if(!$emp) $emp = 1;
$empres = busca($_GET['id'],'facturas','f_id','f_empresa');
if(!$empres) $empres = 1;
...
/*
   ...
   */
     ...
     $version =  "4.0";
     $sellocfd =  sacarcadena('Sello="','"',file_get_contents('../cfd/'.$empres.'/xml/'.busca($id,'facturas','f_id','f_folio').'.xml'));
     $foliosat =  sacarcadena('SelloSAT="','"',file_get_contents('../cfd/'.$empres.'/xml/'.busca($id,'facturas','f_id','f_folio').'.xml'));
     $uuid =  sacarcadena('UUID="','"',file_get_contents('../cfd/'.$empres.'/xml/'.busca($id,'facturas','f_id','f_folio').'.xml'));
     $certisat =  sacarcadena('NoCertificadoSAT="','"',file_get_contents('../cfd/'.$empres.'/xml/'.busca($id,'facturas','f_id','f_folio').'.xml'));
     $certicsd =  sacarcadena('NoCertificado="','"',file_get_contents('../cfd/'.$empres.'/xml/'.busca($id,'facturas','f_id','f_folio').'.xml'));
     $fechatimbrado =  sacarcadena('FechaTimbrado="','"',file_get_contents('../cfd/'.$empres.'/xml/'.busca($id,'facturas','f_id','f_folio').'.xml'));
     $fechaemision =  sacarcadena('Fecha="','"',file_get_contents('../cfd/'.$empres.'/xml/'.busca($id,'facturas','f_id','f_folio').'.xml'));
...
$id = $_GET['id'];
$facturas = $_GET['id'];
$empresa = busca($_GET['id'],'facturas','f_id','f_empresa');
...
$estado = clearvmayus(busca($cliente->estado, 'estado', 'e_id', 'e_nmb'));
$domcliente = $cliente->calle.' '.$cliente->nume.' '.$cliente->colonia.' '.$cliente->municipio.' '.$estado.' '.$cliente->pais.' '.$cliente->cp;
...
if(busca($id,'factura_relacion','fr_origen','COUNT(*)') > 0){
  ...
  $pdf->Ln(7);
  $pdf->Cell(5);
  $pdf->Cell(185,5,"Facturas Relacionadas",1,0,"C",1);
  $pdf->Ln();
  $pdf->SetFont('Arial','',7);
  $pdf->SetTextColor(255,255,255);
  $pdf->Cell(5);
  ...
  while($rwr = $resultr->fetch_array()){
    $folior = busca($rwr['fr_factura'],'facturas','f_id','f_folio');
    $uuidr = $rwr['fr_uuid'];
...
$sqld = 'SELECT * FROM facturasd WHERE d_factura = "'.$id.'" ORDER BY d_id ASC';
...
while($row= $result->fetch_array()){
  ...
  $iva = busca($facturas,'facturas','f_id','f_iva');
  $reti = busca($facturas,'facturas','f_id','f_retiva');
  $isr = busca($facturas,'facturas','f_id','f_isr');
...
//  echo $totalf.'<<<';
  ...
  $pdf->Cell(95,3,utf8_decode("CONDICIÓN DEL PAGO: ".$facts->condiciones),"R",0,"R");
    ...
    $uso = busca($id,'facturas','f_id','f_uso');
...
*/
     ...
     $sellocfd =  sacarcadena('Sello="','"',file_get_contents('../cfd/'.$empresa.'/xml/'.busca($id,'facturas','f_id','f_folio').'.xml'));
     $foliosat =  sacarcadena('SelloSAT="','"',file_get_contents('../cfd/'.$empresa.'/xml/'.busca($id,'facturas','f_id','f_folio').'.xml'));
     $uuid =  sacarcadena('UUID="','"',file_get_contents('../cfd/'.$empresa.'/xml/'.busca($id,'facturas','f_id','f_folio').'.xml'));
     $certisat =  sacarcadena('NoCertificadoSAT="','"',file_get_contents('../cfd/'.$empresa.'/xml/'.busca($id,'facturas','f_id','f_folio').'.xml'));
     $certicsd =  sacarcadena('NoCertificado="','"',file_get_contents('../cfd/'.$empresa.'/xml/'.busca($id,'facturas','f_id','f_folio').'.xml'));
     $fechatimbrado =  sacarcadena('FechaTimbrado="','"',file_get_contents('../cfd/'.$empresa.'/xml/'.busca($id,'facturas','f_id','f_folio').'.xml'));
     $fechaemision =  sacarcadena('Fecha="','"',file_get_contents('../cfd/'.$empresa.'/xml/'.busca($id,'facturas','f_id','f_folio').'.xml'));
    ...
    $sqluu = 'UPDATE facturas SET f_uuid = "'.$uuid.'" WHERE f_id = "'.$id.'"';

    if($uuid != "12345678-ABCD-0987-EFGH-A1B2C3D4E5F6") setq($sqluu) or die($sqluu);

    $u8d = substr($sellocfd,-8);
    QRcode::png('https://verificacfdi.facturaelectronica.sat.gob.mx/default.aspx?id='.$uuid.'&re='.busca($empresa,'empresas','e_id','e_rfc').'&rr='.$cliente->rfc.'&tt='.number_format($totalf,2,'.','').'&fe='.$u8d,'../cfd/'.$empresa.'/qr/'.busca($id,'facturas','f_id','f_folio').'.png');
    $pdf->Image('../cfd/'.$empresa.'/qr/'.busca($id,'facturas','f_id','f_folio').'.png',170,($y-15),30,30);
//    $pdf->Output('../cfd/'.$empresa.'/pdf/'.busca($id,'facturas','f_id','f_folio').'.pdf');

    $sql = 'SELECT COUNT(*) FROM carta_porte WHERE cp_factura = "'.$id.'"';
    $result = setq($sql) or die($sql);
    list($cpnum) = $result->fetch_array();
    if($cpnum > 0){
      $facts->resultcp($id);
      $viaes = array("01"=>"Autotransporte","02"=>"Transporte Marítimo","03"=>"Transporte Aéreo","04"=>"Transporte Ferroviario");

      $pdf->AddPage();
      $pdf->SetFont('Arial','B',9);
      $pdf->SetTextColor(255,255,255);
      $pdf->Cell(190,5,"Carta Porte",1,0,"C",1);

      $pdf->SetFont('Arial','',9);
      $pdf->Ln();
      $pdf->Cell(25,4,"Internacional",1,0,"C",1);
      $pdf->SetTextColor(0,0,0);
      $pdf->Cell(10,4,utf8_decode($facts->TranspInternac),"1");
      $pdf->Cell(25,4,utf8_decode($facts->EntradaSalidaMerc),1,0,"C");
      $pdf->Cell(15,4,utf8_decode($facts->PaisOrigenDestino),1,0,"C");
      $pdf->Cell(25,4,utf8_decode($viaes[$facts->ViaEntradaSalida]),1,0,"C");
      $pdf->SetTextColor(255,255,255);
      $pdf->Cell(20,4,"Peso Bruto",1,0,"C",1);
      $pdf->SetTextColor(0,0,0);
      $pdf->Cell(30,4,utf8_decode(number_format($facts->PesoBrutoTotal,2).' '.$facts->UnidadPeso),"1",0,"R");
      $pdf->SetTextColor(255,255,255);
      $pdf->Cell(25,4,"Total Mercancias",1,0,"C",1);
      $pdf->SetTextColor(0,0,0);
      $pdf->Cell(15,4,utf8_decode($facts->NumTotalMercancias),"1",0,"R");

      $pdf->Ln(8);
      $pdf->SetFont('Arial','B',9);
      $pdf->SetTextColor(255,255,255);
      $pdf->Cell(190,5,"Transporte",1,0,"C",1);
      $pdf->SetFont('Arial','',9);
      $pdf->Ln();
      $pdf->Cell(30,4,"Tipo Transporte",1,0,"C",1);
      $pdf->SetTextColor(0,0,0);
      $pdf->Cell(160,4,utf8_decode($facts->configtransporte.' '.busca($facts->configtransporte,'catcp_configtransporte','ccc_clave','ccc_nmb')),1,0,"C");
      $pdf->Ln();
      $pdf->SetTextColor(255,255,255);
      $pdf->Cell(15,4,"Placa",1,0,"C",1);
      $pdf->SetTextColor(0,0,0);
      $pdf->Cell(20,4,$facts->PlacaVM,1,0,"C");
      $pdf->SetTextColor(255,255,255);
      $pdf->Cell(15,4,utf8_decode("Año"),1,0,"C",1);
      $pdf->SetTextColor(0,0,0);
      $pdf->Cell(10,4,$facts->AnioModeloVM,1,0,"C");
      $pdf->SetTextColor(255,255,255);
      $pdf->Cell(30,4,utf8_decode("Aseguradora"),1,0,"C",1);
      $pdf->SetTextColor(0,0,0);
      $pdf->Cell(100,4,$facts->AseguraRespCivil,1,0,"C");

      $pdf->Ln();
      $pdf->SetTextColor(255,255,255);
      $pdf->Cell(15,4,utf8_decode("Póliza"),1,0,"C",1);
      $pdf->SetTextColor(0,0,0);
      $pdf->Cell(45,4,$facts->PolizaRespCivil,1,0,"C");
      $pdf->SetTextColor(255,255,255);
      $pdf->Cell(50,4,utf8_decode("Número permiso SCT"),1,0,"C",1);
      $pdf->SetTextColor(0,0,0);
      $pdf->Cell(80,4,$facts->numpermiso,1,0,"C");
      $pdf->Ln();
      $pdf->SetTextColor(255,255,255);
      $pdf->Cell(40,4,utf8_decode("Tipo permiso SCT"),1,0,"C",1);
      $pdf->SetTextColor(0,0,0);
      $pdf->Cell(150,4,utf8_decode(busca($facts->tipopermiso,'catcp_tipopermiso','ccp_clave','ccp_nmb')),1,0,"C");
      $pdf->Ln(8);

      $figuratr= array("01"=>"Operador","02"=>"Propietario","03"=>"Arrendador","04"=>"Notificado");

      $pdf->SetTextColor(255,255,255);
      $pdf->Cell(190,5,"Figura transporte",1,0,"C",1);
      $pdf->Ln();
      $pdf->SetTextColor(255,255,255);
      $pdf->Cell(35,4,utf8_decode("RFC Figura transporte"),1,0,"C",1);
      $pdf->SetTextColor(0,0,0);
      $pdf->Cell(30,4,$facts->RFCFigura,1,0,"C");
      $pdf->SetTextColor(255,255,255);
      $pdf->Cell(40,4,utf8_decode("Nombre Figura transporte"),1,0,"C",1);
      $pdf->SetTextColor(0,0,0);
      $pdf->Cell(85,4,utf8_decode($facts->NombreFigura),1,0,"C");
      $pdf->Ln();
      $pdf->SetTextColor(255,255,255);
      $pdf->Cell(35,4,utf8_decode("Licencia"),1,0,"C",1);
      $pdf->SetTextColor(0,0,0);
      $pdf->Cell(30,4,$facts->NumLicencia,1,0,"C");
      $pdf->SetTextColor(255,255,255);
      $pdf->Cell(40,4,utf8_decode("Figura Transporte"),1,0,"C",1);
      $pdf->SetTextColor(0,0,0);
      $pdf->Cell(85,4,$figuratr[$facts->figuratransporte],1,0,"C");
      $pdf->Ln(8);

      $pdf->SetTextColor(255,255,255);
      $pdf->Cell(95,4,utf8_decode("Remolque 1"),1,0,"C",1);
      $pdf->Cell(95,4,utf8_decode("Remolque 2"),1,0,"C",1);
      $pdf->Ln();
      $pdf->SetTextColor(0,0,0);
      $pdf->Cell(75,4,busca($facts->TipoRem1,'catcp_remolques','ccr_clave','ccr_nmb'),1,0,"C");
      $pdf->Cell(20,4,$facts->PlacaRem1,1,0,"C");
      $pdf->Cell(75,4,busca($facts->TipoRem2,'catcp_remolques','ccr_clave','ccr_nmb'),1,0,"C");
      $pdf->Cell(20,4,$facts->PlacaRem2,1,0,"C");


      $pdf->Ln(8);
      $pdf->SetFont('Arial','B',9);
      $pdf->SetTextColor(255,255,255);
      $pdf->Cell(190,5,"Ubicaciones",1,0,"C",1);
      $pdf->SetFont('Arial','',9);
      $pdf->Ln();

      $pdf->Cell(20,4,"Tipo",1,0,"C",1);
      $pdf->Cell(25,4,"RFC",1,0,"C",1);
      $pdf->Cell(85,4,"Nombre",1,0,"C",1);
      $pdf->Cell(40,4,"Fecha Salida/Llegada",1,0,"C",1);
      $pdf->Cell(20,4,"Distancia",1,0,"C",1);
      $pdf->Ln();
      ...
      $pdf->SetTextColor(255,255,255);
        $pdf->Cell(20,4,$rowu['cpu_TipoUbicacion'],1,0,"C",1);
        $pdf->SetTextColor(0,0,0);
        $pdf->Cell(25,4,utf8_decode($rowu['cpu_RFCRemitenteDestinatario']),1,0,"C");
        $pdf->Cell(85,4,utf8_decode($rowu['cpu_NombreRFC']),1,0,"C");
        $pdf->Cell(40,4,$rowu['cpu_FechaHoraSalidaLlegada'],1,0,"C");
        $pdf->Cell(20,4,$rowu['cpu_DistanciaRecorrida'],1,0,"R");
        $pdf->Ln();
        $totaldist+=$rowu['cpu_DistanciaRecorrida'];
        if(!empty($rowu['cpu_CodigoPostal'])){
          $domicilio = $rowu['cpu_Calle'].' '.$rowu['cpu_NumeroExterior'].' '.$rowu['cpu_NumeroInterior'].' '.$rowu['cpu_Colonia'].' '.$rowu['cpu_Municipio'].' '.$rowu['cpu_Estado'].' '.$rowu['cpu_CodigoPostal'];
          $pdf->MultiCell(190,6,$domicilio,1);
        }
      }
      ...
      $pdf->SetFont('Arial','B',9);
      $pdf->SetTextColor(255,255,255);
      $pdf->Cell(190,5,"Mercancias",1,0,"C",1);
      $pdf->SetFont('Arial','',9);
      $pdf->Ln();
      $pdf->Cell(25,4,"Bienes",1,0,"C",1);
      $pdf->Cell(100,4,utf8_decode("Descripción"),1,0,"C",1);
      $pdf->Cell(20,4,"Cantidad",1,0,"C",1);
      $pdf->Cell(20,4,"Unid Medida",1,0,"C",1);
      $pdf->Cell(25,4,"Peso KG",1,0,"C",1);
      $pdf->Ln();
      $pdf->SetFont('Arial','',8);
      $totaldist = 0;
      ...
      $pdf->SetWidths(array('25','100','20','20','25'));

      while($rowu = $facts->resultmer->fetch_array()){
        if($rowu['cpm_ValorMercancia'] > 0) $valorm = $rowu['cpm_ValorMercancia'].' '.$rowu['cpm_Moneda'];
        else $valorm = "No definido";

        $pdf->Row(array($rowu['cpm_BienesTransp'],utf8_decode($rowu['cpm_Descripcion']),utf8_decode($rowu['cpm_Cantidad']),utf8_decode($rowu['cpm_ClaveUnidad']),number_format($rowu['cpm_PesoEnKg'],4)));
      }
      $pdf->Ln();
