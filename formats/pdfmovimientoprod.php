<?php
  ...
  class PDF extends FPDF{
    ...
    var $aligns;
    function Header(){
      $sql='SELECT * FROM pr_movimientos WHERE pm_id="'.$_GET['id'].'" AND pm_tipo = "'.$_GET['tipo'].'"';
      ...
      include_once('../modulos/movimientosprod.php');
      $movim = new modelmovimientosprod();
      ...
      // T�tulo
      $folio = busca($_GET['id'],'pr_movimientos','pm_tipo = "'.$_GET['tipo'].'" AND pm_id','pm_folio');
      $motivo = busca($_GET['id'],'pr_movimientos','pm_tipo = "'.$_GET['tipo'].'" AND pm_id','pm_motivo');

      $tipomov = array("E"=>"Entrada","S"=>"Salida","T"=>"Trasapaso");
      //imprimir el numero de la cotizacion
      $this->SetFont('kalingab','',18);
      $this->Cell(35);
      $this->Cell(90,32,utf8_decode($tipomov[$_GET['tipo']].' de Mercancía en Producción'),0,0,"C");
      $this->Ln();
      ...
      $this->SetFillColor(91, 155, 213);
      $this->SetTextColor(255,255,255);
      $this->Cell(41,6,utf8_decode("Fecha de captura:"),1,0,'C',true);
      $this->SetTextColor(0,0,0);
...
$pdf->Cell(4);
$sql='SELECT * FROM pr_movimientosd INNER JOIN pr_materiaprima ON pmd_materiaprima = pmp_id WHERE pmd_movimiento="'.$movim->id.'" AND pmd_tipo = "'.$movim->tipo.'"';
...
while($row=$resultado->fetch_array()){
  $iva=0;    //cuando se requiere desplegar el iva
    ...
    $pdf->RowMin(array($row['pmd_cantidad'],utf8_decode($nombre)),false);
    $bandera=!$bandera;
}
