...
foreach($titulosColumnas as $titc){
  ...
  while($rowal = $resultrem->fetch_array()){
    ...
    WHERE rc_remision = "'.$rowal['r_id'].'" AND rc_ligado IS NULL GROUP BY rc_articulo, rc_modelo '; 
    ...
    $objPHPExcel->setActiveSheetIndex(0)->setCellValue("A".$i, $s);
    $objPHPExcel->getActiveSheet()->getStyle("A".$i)->applyFromArray($Crosa);
