...
var $aligns; 
  ...
  $this->Cell(35);
  $this->Cell(110,32,utf8_decode('REPORTE DE FORMAS DE PAGO'),0,0,"C");
  $this->Ln();
...
$pdf->Cell($col6,7,'Monto',1,0,'C',true);
$pdf->Cell($col7,7,utf8_decode('Comisión'),1,0,'C',true);
$pdf->Cell($col8,7,'Total ingreso',1,0,'C',true);
$pdf->Cell($col9,7,'Forma de pago',1,0,'C',true);
$pdf->Ln(7);
...
while($rowal = $result->fetch_array()){
  ...
  if($fpago != "EC" && $fpago != "TD" && $fpago != "TC" && $rowal['tipo'] == "I"){
    ...
    $pdf->RowMin(array(utf8_decode($folio),fecha_formato($rowal['fecha'],true,true),$nmbencargado,utf8_decode($nmbcl), number_format($totalrem,2), number_format($rowal['monto'],2), number_format($rowal['comision'],2), number_format($totaling,2), utf8_decode($rowal['fpago'])),false);
    $total+=$rowal['monto'];
  }
  if(($fpago == "" || $fpago == "EC" || $fpago == "TD" || $fpago == "TC") && $rowal['tipo'] == "C"){
    ...
    if(($fpago == "" || $fpago == "EC") && $efectivo > 0){
      ...
      $pdf->RowMin(array(utf8_decode($folio),fecha_formato($rowal['fecha'],true,true),$nmbencargado,utf8_decode($nmbcl), number_format($totalrem,2), number_format($efectivo,2), number_format('0',2), number_format($efectivo,2), utf8_decode('EFECTIVO EN CAJA')),false);
      $total+=$efectivo;
    }
    if(($fpago == "" || $fpago == "TC" && $comision > 0) && $tarjeta > 0){
      ...
      $totalingresos += $totaling;
      $pdf->RowMin(array(utf8_decode($folio),fecha_formato($rowal['fecha'],true,true),$nmbencargado,utf8_decode($nmbcl), number_format($totalrem,2), number_format($tarjeta,2), number_format($comision,2), number_format($totaling,2), utf8_decode('TARJETA DE CRÉDITO EN CAJA')),false);
      $total+=$tarjeta;
    } else if(($fpago == "" || $fpago == "TD") && $tarjeta > 0){
      $pdf->RowMin(array(utf8_decode($folio),fecha_formato($rowal['fecha'],true,true),$nmbencargado,utf8_decode($nmbcl), number_format($totalrem,2), number_format($tarjeta,2), number_format('0',2), number_format($tarjeta,2), utf8_decode('TARJETA DE DÉBITO EN CAJA')),false);
      $total+=$tarjeta;
    }
  }
}

$pdf->RowMin(array('','','','', 'Total:', number_format($total, 2), number_format($totalcomision, 2), number_format($totalingresos, 2), ''),false);
