...
//   $_SESSION['emp'] = 1;
   $sellocfd =  sacarcadena('Sello="','"',file_get_contents('../cfd/'.$_SESSION['emp'].'/xml/nomina/'.$nom->folio.'.xml'));
   $foliosat =  sacarcadena('SelloSAT="','"',file_get_contents('../cfd/'.$_SESSION['emp'].'/xml/nomina/'.$nom->folio.'.xml'));
   $uuid =  sacarcadena('UUID="','"',file_get_contents('../cfd/'.$_SESSION['emp'].'/xml/nomina/'.$nom->folio.'.xml'));
   $certisat =  sacarcadena('NoCertificadoSAT="','"',file_get_contents('../cfd/'.$_SESSION['emp'].'/xml/nomina/'.$nom->folio.'.xml'));
   $fechatimbrado =  sacarcadena('FechaTimbrado="','"',file_get_contents('../cfd/'.$_SESSION['emp'].'/xml/nomina/'.$nom->folio.'.xml'));
   $salarioct =  sacarcadena('SalarioBaseCotApor="','"',file_get_contents('../cfd/'.$_SESSION['emp'].'/xml/nomina/'.$nom->folio.'.xml'));
   $salariodi =  sacarcadena('SalarioDiarioIntegrado="','"',file_get_contents('../cfd/'.$_SESSION['emp'].'/xml/nomina/'.$nom->folio.'.xml'));
   ...
   $this->SetFont('Arial','',7);
   $this->Cell(40,4,date('Y-m-d',strtotime($nom->ffin)),"R",0,0);
   $this->Ln();
...
function Footer()
{
   ...
   $salariodi =  sacarcadena('SalarioDiarioIntegrado="','"',file_get_contents('../cfd/'.$_SESSION['emp'].'/xml/nomina/'.$nom->folio.'.xml'));
    ...
    if (!is_dir('cfd/'.$_SESSION['emp'].'')) {
     ...
     if (!is_dir('../cfd/'.$_SESSION['emp'].'/qr')) {
        @mkdir('../cfd/'.$_SESSION['emp'].'/qr', 0777);
    }
     if (!is_dir('../cfd/'.$_SESSION['emp'].'/qr/nomina')) {
        @mkdir('../cfd/'.$_SESSION['emp'].'/qr/nomina', 0777);
    }

    $y = $yd+35;
    QRcode::png('?re='.busca("1",'empresas','e_id','e_rfc').'&rr='.busca($empleado,'empleados','e_id','e_rfc').'&tt='.number_format($total,2,'.','').'&id='.$uuid,'../cfd/'.$_SESSION['emp'].'/qr/nomina/'.$nom->folio.'.png');
    $pdf->Image('../cfd/'.$_SESSION['emp'].'/qr/nomina/'.$nom->folio.'.png',145,$y,50,50);
    ...
    if (!is_dir('../cfd/'.$_SESSION['emp'].'/pdf')) {
        @mkdir('../cfd/'.$_SESSION['emp'].'/pdf', 0777);
    }

    if (!is_dir('../cfd/'.$_SESSION['emp'].'/pdf/nomina')) {
        @mkdir('../cfd/'.$_SESSION['emp'].'/pdf/nomina', 0777);
    }

    $pdf->Output();
    $pdf->Output('../cfd/'.$_SESSION['emp'].'/pdf/nomina/'.$nom->folio.'.pdf');
    $pdf->Close();
?>