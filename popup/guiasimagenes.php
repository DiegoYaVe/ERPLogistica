...
if($tipo == "P") {
  $id = $_GET['guia'];
  $nmb = busca($id, 'guias_articulos', 'ga_id', 'ga_nmb');
  $titulo = "de la guía de la preparación";
  $ruta = 'img/guias/preparacion/';
  /* $estatus = busca($id, 'guias_articulos', 'ga_id', 'ga_estatus'); */
}else {
  if(isset($_GET['art'])){
    $id = $_GET['em'];
    $art = $_GET['art'];
    ...
    $articulo = busca($art, 'remisionesc', 'rc_id', 'rc_articulo');
    $modelo = busca($art, 'remisionesc', 'rc_id', 'rc_modelo');
    $numero = busca($art, 'remisionesc', 'rc_id', 'rc_numero');
    
    $artname = busca($articulo, "articulos", "a_id", "a_nmb");
    $var = busca($articulo, 'articulos_variantes', 'av_modelo = "' . $modelo . '" AND av_articulo', 'COUNT(*)');
    if ($var > 0) {
      $artname .= " " . busca($articulo, 'articulos_variantes', 'av_modelo = "' . $modelo . '" AND av_articulo', 'av_nmb');
    }
    $artname .= " " . $numero;
    $nmb = $artname;
  
    $titulo = "del producto ".$nmb." del embarcamiento";
    $nmb = '';
  } else{
    $art = '';
    $id = $_GET['guia'];
    $tipo = $_GET['tipo'];
    $nmb = busca($id, 'guias_articulos', 'ga_id', 'ga_nmb');
    $titulo = "de la guía del embarcamiento";
    $ruta = 'img/guias/preparacion/';
    /* $estatus = busca($id, 'guias_articulos', 'ga_id', 'ga_estatus'); */
  }
...
<div class="container col-10">
  <div class="alert alert-primary"> Fotografías <?php echo $titulo;?> <?php echo $nmb;?></div>
    <div class="mt-4 row">
        <?php
        if(isset($_GET['art'])){
          $sql = 'SELECT * FROM guias_imagenes WHERE gi_cccid = "'.$art.'" AND gi_tipo = "'.$tipo.'" ORDER BY gi_orden DESC';
        } else{
          $sql = 'SELECT * FROM guias_imagenes WHERE gi_guia = "'.$id.'" AND gi_tipo = "'.$tipo.'" ORDER BY gi_orden DESC';
        }          
          $result = setq($sql) ;
          $resultn = setq($sql) ;
...
$('#formimagenes').submit(function(event) {
  ...
  $.ajax({
    ...
    url: 'query/cargaimagenes.php', // URL a la que deseas enviar la solicitud
