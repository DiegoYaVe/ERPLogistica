...
?>
    ...
    <div class="container">
    <div class="col-12 alert alert-primary">Insertar una nueva garantía</div>
    <div class="col-12 alert alert-danger">Este apartado es exclusivo de las garantías que no tienen una remisión previamente existente en el sistema</div>
      <form class="row" method="post" action="index?modulo=fabricagarantias&accion=insertgarantia&garantia=1" enctype="multipart/form-data" id="formsetgarantia">

      <!-- Inicia campo de de busqueda para el cliente -->
      <div class="col-md-9">
        <div class="mb-5">  
          <label>Cliente: </label>
            <input type="text" name="idcliente" id="idcliente" value="" placeholder="Nombre o alias del cliente" class="search_query form-control" required>
            <input type="hidden" value="" name="cliente" id="cliente">
        </div>
      ...
      <!-- Finaliza campo de de busqueda para el cliente -->
    ...
    echo '<div class="row nowrap">';    
      ...
      <div class="col-12 col-md-12 alert alert-primary" id="titulodir">Ingrese una dirección del cliente</div>';
        ...
        <div class="row" id="selectdir">
          ...
          <div class="col-12 col-md-6" id="selectdirecciones">
            <div class="mb-5">
        ...
        echo '<div class="row" id="formdir">
          ...
          <div class="col-12 col-md-3" id="divcp">
            <div class="mb-5">
              <label >Código postal</label>
              <input type="text" value="'.$cp.'" class="form-control" onchange="colonia();" id="cp" name="cp"  '.$disall.' required>
              <div id="nameError" class="error-message"></div>
            </div>
          </div>
          <div class="col-12 col-md-5" id="divcolonia">
            <div class="mb-5">
              <label >Colonia</label>
              <select id="col" name="col" value="'.$colonia.'" class="form-control"  '.$disall.' required>
                <option value="" disabled selected>Seleccione la colonia</option>
              </select>
              <input type="hidden" value="'.$colonia.'" name="coldesc" id="coldesc">
            </div>
          </div> 
          <div class="col-12 col-md-4" id="divestado">
            <div class="mb-5">
              <label >Estado</label>
              <input type="text" value="'.$estado.'" class="form-control" id="estado" name="estado"  '.$disall.' required>
              <input type="hidden" value="'.$estadoh.'" class="form-control" id="estadoh" name="estadoh" >
            </div>
          </div>
          <div class="col-12 col-md-4" id="divmunicipio">
            <div class="mb-5">
              <label >Municipio</label>
              <select id="municipio" value = "'.$municipio.'" name="municipio" class="form-control"  '.$disall.' required>
                <option value="" disabled selected>Seleccione el municipio</option>
              </select>
              <input type="hidden" value="'.$municipio.'" name="munidesc" id="munidesc">
            </div>
          </div>
          <div class="col-12 col-md-4" id="divcalle">
            <div class="mb-5">
              <label id="lblcalle">Calle</label>
              <input type="text" value="'.$calle.'" class="form-control" id="calle" name="calle"  '.$disall.' required>
            </div>
          </div>
          <div class="col-12 col-md-4" id="divpais" hidden>
      ...
      <div class="mb-5 col-md-4">
        <label>Folio:</label>
          <input type="text" class="form-control" name="folio" id="folio" placeholder="Folio de la nota de venta" required>
        </div>
        <div class="mb-5 col-md-4">
        <label>Artículo:</label>
          <input type="text" class="form-control" id="articulo" name="articulo" placeholder="Nombre del artículo" required>
        </div>
        <div class="mb-5 col-md-4">
        <label>Modelo:</label>
          <input type="text" class="form-control" id="modelo" name="modelo" placeholder="Modelo del artículo">
        </div>
        <div class="mb-5 col-md-12">
        <label>Descripción:</label>
          <textarea class="form-control" name="descripcion" id="descripcion" placeholder="Descripción del ingreso a la garantía" required></textarea>
        </div>
...
<script>
    //Inician funciones del nombre del cliente
...
$('#idcliente').on('keyup', function() {
      ...
      $('.suggest-element').on('click', function(){
        ...
        $.ajax({
          ...
          success: function(dataRTN) {
            ...
            traerdirecciones(cliente.value);
          }
        });
        //Editamos el valor del input con data de la sugerencia pulsada
        $('#idcliente').val($('#'+id).attr('data'));
  ...
  //Finalizan funciones del nombre del cliente
  ...
  if(seldir.value == ""){
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        colonia();
        if(pais.value != 146 & pais.value !== ''){
          console.log("Pais != 146");
          envioextranjero2(true, pais.value);
        } else if(pais.value == 146){
          console.log("Pais == 146");
...
function traerdirecciones(cliente){
  $.ajax({
      type: "POST",
      url: "query/traerdirecciones.php",
      dataType: 'html',
      data: {"cliente": cliente},
      success: function(response) {
        $('#selectdirecciones').html(response);
        cambiardir();
      }
    });
}
