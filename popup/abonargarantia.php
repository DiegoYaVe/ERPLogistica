...
include_once('../modulos/fabricagarantias.php');
$remm = new modelfabricagarantias();

include_once('../modulos/cxcobrar.php');
$cxc = new modelcxcobrar();

$remm -> select($garantia);
$estatusp = array("N"=>"NUEVA","F"=>"LIQUIDADA","C"=>"CANCELADA","A"=>"ABONADA");
$estatusa = array("P"=>"POR APROBAR","F"=>"APROBADO", "C"=> "CANCELADO","A"=>"APROBADO");
$sql = 'SELECT cx_id, cx_importe, cx_abonado, cx_estatus FROM cxcobrar WHERE cx_tipo = "B" AND cx_referencia = "'.$garantia.'"';
$result = setq($sql);
...
/* $sqlb = 'SELECT SUM(cd_total) FROM cajasd WHERE cd_garantia = "'.$garantia.'" AND cd_estatus="P"';
$resultb = setq($sqlb);
  ...
  <div class="container col-10">  
    ...
    <div class="row">
      <div class="col-md-12 col-sm-12 mt-4" id="form" '.$form.'>
        ...
        <form action="?modulo=fabricagarantias&accion=abonargarantia'.$linkurl.'" method="post" enctype="multipart/form-data" onsubmit="checksubmit();">
          <input id="cxc" name="idcxc" type="hidden" value="'.$idcxc.'">
          <input id="garantia" name="garantia" type="hidden" value="'.$garantia.'">  
          <div class="row">
            <div class="col-12 col-md-2">
              <div class="mb-5">
                <label for"nmbac">Importe del ingreso</label>
                <input type="number" min="1" step="0.01" name="importe" value="'.round($rest, 2).'" id="nmb" class="form-control" placeholder="Importe del ingreso" required="required" onfocus="this.select();" />
              </div>
            </div>
            <div class="col-12 col-md-2">
              <div class="mb-5">
            ...
            <div class="col-12 col-md-6">
              <div class="mb-5">
                ...
                <input type="text" name="observaciones" id="observaciones" value="PAGO DE GARANTÍA DE LA REMISIÓN '.$remm->folio.'" class="form-control" placeholder="Observaciones" onfocus="this.select();"/>
              </div>
            </div>
            ';
        ...
        <center>
          <table class="table table-striped table-hover table-bordered" style="width:100%">
            ...
            <tbody>';
              ...
              /* $sql = 'SELECT * FROM cajasd WHERE cd_garantia = "'.$remm->id.'" AND cd_estatus != "C"';
              $resultab2 = setq($sql);
