...
$preciosend = $rowcc['cc_precioenvio'];
...
$client = busca($rowcc['cc_tablero'], 'crm_tableros', 'ct_id', 'ct_cliente');
...
$country = intval(busca($rowcc['cc_direnvio'], 'crm_direcciones', 'cd_id', 'cd_pais'));
$dirsend = '';
$flagcountry = 0;
if ($country != 146 && $country != 0) {
  $dirsend = busca($rowcc['cc_direnvio'], 'crm_direcciones', 'cd_id', 'cd_calle');
  $flagcountry = 1; //El país es diferente de México
} else {
  $dirsend = busca($rowcc['cc_direnvio'], 'crm_direcciones INNER JOIN estado ON cd_estado = e_id', 'cd_id', 'CONCAT(cd_calle," ",cd_nume," ",cd_numi,", ",cd_colonia,", ",cd_municipio,", ",e_nmb,", C.P.:",cd_cp)');
}
$pais = busca($country, 'paises', 'p_id', 'p_nmb');
$ndir = $dirsend . ', ' . $pais;
$cliente = busca($rowcc['cc_tablero'], 'crm_tableros', 'ct_id' ,'ct_cliente');
$clientedir =  busca($rowcc['cc_direnvio'], 'crm_direcciones', 'cd_id' ,'cd_cliente');
if($cliente != $clientedir) {
  $sqlupd = 'UPDATE crm_cotizaciones SET cc_direnvio = "0" WHERE cc_id = "'.$idcotiza.'"';
  setq($sqlupd);
  $ndir = 'SIN DIRECCIÓN REGISTRADA';
}
echo '
      <span>Direccion: ' . $ndir. '</span><br>
...
function imprimircheckboxes($k, $articulotipo, $cob, $reado, $reado2, $padre = NULL)
{
  ...
  $retorno .= '
          <td>
            <input style="font-size: 12px;" class="form-control" type="date" name="fechaEnvio' . $k . '" id="fechaEnvio' . $k . '" value="' . $fenvio . '" required>
          </td>';
...
while ($row0 = $result0->fetch_array()) {
  ...
  $sqlcmb = 'SELECT a_id, a_nmb, a_tipoprod, cdm_modelo, cdm_cantidad, cdm_id, cdm_articulo 
  ...
  if ($tipoprod == "M") {
    for ($j = 1; $j <= intval($cantidad); $j++) {
      ...
      while ($rowcb = $resultd->fetch_array()) {
        $var = intval(busca($rowcb['ccc_articulo'], 'articulos_variantes', 'av_modelo = "' . $rowcb['ccc_modelo'] . '" AND av_articulo', 'COUNT(*)'));
        ...
        $artname .= " ".$rowcb['ccc_numero'];

        $p1 = $rowcb['ccc_id'];
        $p2 = $rowcb['ccc_tipoenvio'];
        $peso = busca($rowcb['ccc_articulo'], 'articulos', 'a_id', 'a_peso');
        $ligados = $rowcb['ccc_ligado'];
        if (!empty($ligados)) {
          //Inicia listado de artículos ligados de este artículo que es un paquete con cantidad de uno
          $sqligados = 'SELECT ccc_articulo, ccc_modelo, ccc_numero FROM crm_cotizacionesc WHERE ccc_id = "' . $ligados . '"';
          ...
          $var = intval(busca($rowligado['ccc_articulo'], 'articulos_variantes', 'av_modelo = "' . $rowligado['ccc_amodelo'] . '" AND av_articulo', 'COUNT(*)'));
          if ($var > 0) {
            $artnameligado .= " " . busca($rowligado['ccc_articulo'], 'articulos_variantes', 'av_modelo = "' . $rowligado['ccc_modelo'] . '" AND av_articulo', 'av_nmb');
          }
          $artnameg = $artname." DEL ".$artnameligado . " " . $rowligado['ccc_numero'];
    ...
    for ($i = 0; $i < intval($cantidad); $i++) {
      ...
      $p1 = busca($cdmarticulo2, "crm_cotizacionesc", "ccc_cotizacion = '" . $idcotiza . "' AND ccc_cotizaciond = '" . $cdmid . "' AND ccc_modelo = '" . $modelo2 . "' AND ccc_numero = '" . ($i + 1) . "' AND ccc_articulo", "ccc_id");
      $p2 = busca($cdmarticulo2, "crm_cotizacionesc", "ccc_cotizacion = '" . $idcotiza . "' AND ccc_cotizaciond = '" . $cdmid . "' AND ccc_modelo = '" . $modelo2 . "' AND ccc_numero = '" . ($i + 1) . "' AND ccc_articulo", "ccc_tipoenvio");
      $peso = busca($cdmarticulo2, 'articulos', 'a_id', 'a_peso');
/* 
      $ligado = busca2($p1, 'crm_cotizacionesc', 'ccc_ligado', 'ccc_id');
      echo $ligado;
      echo '<br>';
      echo '<br>'; */
      $ligado = busca($p1, 'crm_cotizacionesc', 'ccc_ligado', 'ccc_id');
      $numero = busca($p1, 'crm_cotizacionesc', 'ccc_id', 'ccc_numero');
      ...
      if (!empty($ligado)) {
        //Inicia listado de artículos ligados de este artículo que es un paquete con cantidad de uno
        $sqligados = 'SELECT ccc_id, ccc_tipoenvio, ccc_articulo, ccc_modelo, ccc_numero FROM crm_cotizacionesc WHERE ccc_ligado = "' . $p1 . '"';
        $resultligado = setq($sqligados);
        while($rowligado = $resultligado->fetch_array()){
        $p12 = $rowligado['ccc_id'];
        $p22 = $rowligado['ccc_tipoenvio'];
        $artnameligado = busca($rowligado['ccc_articulo'], "articulos", "a_id", "a_nmb");
        $var = intval(busca($rowligado['ccc_articulo'], 'articulos_variantes', 'av_modelo = "' . $rowligado['ccc_amodelo'] . '" AND av_articulo', 'COUNT(*)'));
        if ($var > 0) {
          $artnameligado .= " " . busca($rowligado['ccc_articulo'], 'articulos_variantes', 'av_modelo = "' . $rowligado['ccc_modelo'] . '" AND av_articulo', 'av_nmb');
        }
        $artnameg = $artnameligado . " " . $rowligado['ccc_numero'] . " DEL " . $artname;
        $peso = busca($rowligado['ccc_articulo'], 'articulos', 'a_id', 'a_peso');
...
echo '
  ...
  <div class="col-' . $tamanio . ' col-md-' . $tamanio . '">
      <h2><label for="precio">Costo del envío' . $text . '</label></h2>
      <div class="input-group mb-3">
          <div class="input-group-prepend">
              <span class="input-group-text">' . $moneda . '</span>
          </div>
          <input hidden type="text" name="id" id="id" value="' . $_GET['id'] . '"/>
          <input type="number" name="precio" min="0" step="0.01" id="precio" class="form-control" placeholder="Costo del envío de la cotización" required="required" onfocus="this.select();" />
      </div>
  </div>';
