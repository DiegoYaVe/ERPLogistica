<?php
  ...
  function imprimircheckboxes($k, $articulotipo, $background, $idremision, $producto, $peso) {                    
    ...
    //Cargamos las guías
    $nguias = 'SELECT ag_guia FROM articulos_guias WHERE ag_cccid="'.$k.'"';
    ...
    $rtn .= '
        <div style="padding-top: 5px; display: flex; align-items: center;">                     
          <input placeholder="Ingrese el número de guía" class="form-control" type="text" name="in'.$k."2".'" id="in'.$k."2".'" value="'.$guia2.'" readonly>
          &nbsp;&nbsp;
        </div>';
    }
  ...
  <form action="?modulo=guias&accion=setenviosgeneral" method="post" onsubmit="checksubmit();" enctype="multipart/form-data">
  <div class="row">
  ...
  <center>
    ...
    <!-- <form method="post" id="formenvios" action="?modulo=cotizaciones&accion=seleccionarenvio" autocomplete="off" class="mt-2 container"> -->
    ...
    while($row0 = $result0->fetch_array()){
      ...
      $queryd = 'SELECT ccc_articulo, ccc_modelo, COUNT(*) as cantidad, ccc_id, ccc_cotizacion, ccc_cotizaciond, 
                          ...
                          WHERE ccc_cotizacion = "'.$idcotiza.'" AND ccc_cotizaciond = "'.$row0['ccc_cotizaciond'].'" AND ccc_combo = "'.$j.'" AND ccc_tipoenvio != "C"
    ...
    <input type="hidden" value="'.$tcotiza.'" name="idcotiza" id="idcotiza">
    <input type="hidden" value="'.$tremision.'" name="idremision" id="idremision">
    </form>
    </div>
  </center>
  </div>';
