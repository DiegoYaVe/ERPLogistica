...
<body id="kt_body"
	...
	<div class="d-flex flex-column flex-root">
		...
		<!--begin::Brand-->
		<div class="text-center" id="kt_aside_logo">
		<!--begin::Logo-->
		<a href="index.php">
			<img alt="Logo" src="img/UNICO-LOGO-OFICIAL.png" class="h-65px logo" />
		</a>
		<!--end::Logo-->
		<!--begin::Aside toggler-->
		<div id="kt_aside_toggle" class="btn btn-icon w-auto px-0 btn-active-color-primary aside-toggle " hidden
...
/* echo "Registros: ".$contador; */
	...
	<!-- INICIA MEGA MENU -->
		...
		<div id="kt_header_search" class="d-flex align-items-stretch" data-kt-search-keypress="true"
			...
			<div data-kt-search-element="content"
				class="menu menu-sub menu-sub-dropdown p-7 w-600px w-lg-625px">
				<!--begin::Wrapper-->
		...
		<div class="d-flex align-items-center ms-1 ms-lg-3">
											<!--begin::Menu- wrapper-->
      <a data-fancybox data-type="ajax" data-src="popup/tablerosvendedor.php" href="javascript:;" class="d-flex text-dark text-hover-primary align-items-center">
																	<!--begin::Symbol-->
          <span class="symbol-label bg-light">
            <span class="svg-icon svg-icon-2 svg-icon-primary">
      			  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
      ...
      <div class="btn btn-icon btn-active-light-primary position-relative w-30px h-30px w-md-40px h-md-40px" data-kt-menu-trigger="click" data-kt-menu-attach="parent" data-kt-menu-placement="bottom-end">
											...
											<!--begin::Menu-->
											<div class=" menu menu-sub menu-sub-dropdown menu-column w-600px w-lg-600px" data-kt-menu="true">
												<!--begin::Heading-->
												<div class="d-flex flex-column bgi-no-repeat rounded-top" style="background-image:url('assets/media/misc/pattern-1.jpg'); background-size: cover;">
													<!--begin::Title-->
													<h3 class="text-white fw-bold px-9 mt-10 mb-6">Buscador</h3>
													<center>
													<div class="col-10">
														<input type="text" id="busquedatablero" onchange="buscartablero();" onkeyup="buscartablero()" class="form-control form-control-sm" placeholder="Busqueda de tablero"
																			style="border: none; border-radius: 10px; border-bottom: inset;">
													</div>
													</center>
													<!--end::Title-->
													<!--begin::Tabs-->
													<ul class="nav nav-line-tabs nav-line-tabs-2x nav-stretch fw-bold px-9">
														<li class="nav-item">
															<a class="nav-link text-white opacity-75 opacity-state-100 pb-4" data-bs-toggle="tab" href="#kt_topbar_notifications_1" id="resultados">Resultados</a>
														</li>
													</ul>
													<!--end::Tabs-->
												</div>
												...
												<div class="tab-content">
													...
													<div class="tab-pane fade" id="kt_topbar_notifications_1" role="tabpanel">
														...
														<div class="scroll-y mh-325px my-5 px-8">
															<!--begin::Item-->
															<li class="dropdown dropdown-user nav-item noshowtab">
															<div class="table-responsive" bis_skin_checked="1" id="tabletableros">
														...
														<script>
															function buscartablero(){
																var palabra = document.getElementById("busquedatablero");
																var res = document.getElementById("resultados");
																res.click();
																if(palabra.value.length >= 3){
																	$.ajax({
																		method: "POST",
																		url: "query/busquedatableros.php",
																		data: {palabra: palabra.value},
																	}).success(function(data){
																		console.log('devolvio kt_aside_logo');
																		console.log(data);
																		document.getElementById("tabletableros").innerHTML = data;
																	});
																}
															}
														</script>
		...
		<!--end::Chat-->
    <?php 
      $lkactividad = '?modulo=actividades&accion=show';
      $txtactividad = 'Ver Calendario';

      $lkseguimiento = '?modulo=actividades&accion=show';
      $txtseguimiento = 'Seguimiento de proyectos';
    ...
    ?>
		...
		<div class="btn btn-icon btn-active-light-primary w-30px h-30px w-md-40px h-md-40px"
			...
			<a href="#" data-toggle="dropdown" class="nav-link nav-link-label btn position-relative menu-link px-5">
    ...
    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-800 menu-state-bg menu-state-primary fw-bold py-4 fs-6 w-275px"
        ...
        <a data-fancybox data-type="ajax" href="javascript:;" data-src="popup/newactividad.php?act=2&id=2" class=" menu-link px-5"><i class="fas fa-plus"></i>  <span class="menu-title"> Crear Actividad </span></a>
        </div>
        <div class="menu-item px-5">
        <div class="dropdown-divider"></div><a href="<?php echo $lkactividad; ?>" class=" menu-link px-5"><i class="fas fa-calendar-alt"></i><?php echo $txtactividad; ?></a>
        </div>
        <!-- <div class="menu-item px-5">
        <div class="dropdown-divider"></div><a type="button" data-toggle="modal" data-target="#exampleModal22" class="menu-link px-5"><i class="fas fa-briefcase"></i> Ver Folios</a>
        </div> -->
        <!--begin::View more-->
    ...
    <?php
		$sqlfnot = 'SELECT MIN(ca_fecha) FROM crm_actividades INNER JOIN crm_actividadesd ON ca_id = cad_actividad 
		WHERE ca_estatus = "N" AND cad_notificacion = "1"';
		$resultfnot = setq($sqlfnot);
		list($f) = $resultfnot->fetch_array();
		$fini = date('Y-m-d', strtotime($f));
		$ffin = date('Y-m-d',strtotime('next monday'));
		$numnot = 0;
		$sqlcnot = 'SELECT COUNT(*) FROM crm_actividadesd INNER JOIN crm_actividades ON ca_id = cad_actividad 
				WHERE cad_user = "'.$_SESSION['uid'].'" AND cad_notificacion = "1" AND ca_estatus = "N"
				AND DATE(ca_fecha) BETWEEN "'.$fini.'" AND "'.$ffin.'"';
		$resultcnot = setq($sqlcnot);
		list($numnot) = $resultcnot->fetch_array();
    ...
    <div class="d-flex align-items-center ms-1 ms-lg-3" id="kt_header_user_menu_toggle">
		...
		<div class="btn btn-icon btn-active-light-primary w-30px h-30px w-md-40px h-md-40px"
			...
			<!--begin::Svg Icon | path: icons/duotune/general/gen025.svg-->
			<a href="#" data-toggle="dropdown" class="nav-link nav-link-label btn position-relative menu-link px-5">
      <span class="position-absolute top-0 start-75 badge badge-circle fw-bolder fs-7 badge-<?php echo $numtag; ?> tag-up"><?php echo $numnot; ?></span>
    ...
    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-800 menu-state-bg menu-state-primary fw-bold py-4 fs-6 w-275px"
        ...
        <?php
          $sqlnot = 'SELECT crm_actividades.*,crm_acciones.ca_icono icon, crm_actividadesd.cad_id,crm_actividadesd.cad_notificacion
                  FROM crm_actividades INNER JOIN crm_acciones ON crm_actividades.ca_accion = crm_acciones.ca_id
                  INNER JOIN crm_actividadesd ON crm_actividades.ca_id = crm_actividadesd.cad_actividad
                  WHERE crm_actividadesd.cad_user = "'.$_SESSION['uid'].'" AND crm_actividades.ca_estatus = "N"
                  AND (crm_actividadesd.cad_notificacion = "1" OR crm_actividades.ca_fecha = "'.date('Y-m-d').'")
                  AND DATE(ca_fecha) BETWEEN "'.$fini.'" AND "'.$ffin.'" ORDER BY cad_notificacion DESC,ca_fecha';
          $resultnot = setq($sqlnot);
          $i = 0;
          while($rownot = $resultnot->fetch_array()){
            $i++;
            if($rownot['cad_notificacion'] == 1) {$hdn = ''; $backg = 'style="background:#E9E9E9"';}
            else {$hdn = 'hidden'; $backg = '';}
            echo'<a href="?modulo=actividades&accion=index&ida='.$rownot['ca_id'].'" class="list-group-item" '.$backg.'>
              <div class="media">
                <div class="media-left valign-middle">
                  <i class="'.$rownot['icon'].' icon-bg-circle bg-teal"></i>
                </div>
                <div class="media-body">
                  <h7 class="media-heading black darken-3">'.$rownot['ca_nmb'].'</h7>
                  <p class="notification-text font-small-2 text-muted">'.$rownot['ca_descripcion'].'</p>
                  <p class="notification-text font-small-2 text-muted">'.$rownot['ca_fecha'].' '.$rownot['ca_hora'].'</p>
                </div>
                <div class="media-right valign-middle" '.$hdn.'>
                  <i class="icon-circle icon-red"></i>
                </div>
              </div>
            </a>';
          }
      ...
      <!--end::Menu wrapper-->
		...
		<!--end::Quick links-->
		<!--begin::User-->
		<div class="d-flex align-items-center ms-7 ms-lg-7" id="kt_header_user_menu_toggle">
		<!--begin::Menu wrapper-->
		<div class="cursor-pointer symbol symbol-30px symbol-md-40px" data-kt-menu-trigger="click"
			data-kt-menu-attach="parent" data-kt-menu-placement="bottom-end">
			<?php
			$avatar = busca($_SESSION['uid'], 'usuarios', 'u_id', 'u_avatar');
			if(empty($avatar)){
				$avatarg = "images/avatares/blank.webp";
			} else{
				$avatarg = "images/avatares/".$avatar;
			}
			?>
			<img src="<?php echo $avatarg;?>" alt="user" />
		</div>
		<!--begin::Menu-->
		<div
			...
			<div class="menu-item px-3">
				<div class="menu-content d-flex align-items-center px-3">
					...
					<div class="symbol symbol-50px me-5">
						<img alt="Logo" src="<?php echo $avatarg;?>" />
					</div>
					<!--end::Avatar-->
					<!--begin::Username-->
					<div class="d-flex flex-column" style="white-space: nowrap; overflow: auto; text-overflow: ellipsis;">
						<div class="fw-bolder d-flex align-items-center fs-5"><?php echo $_SESSION['uid']?></div>
							<a class="fw-bold text-muted text-hover-primary fs-7">
							<?php
							echo busca($_SESSION['uid'], 'usuarios', 'u_id', 'u_mailcorp');
							?>
						</a>
					</div>
					<!--end::Username-->
			...
			<!--begin::Menu item-->
			<div class="menu-item px-5">
				<a href='?modulo=usuarioconfig&accion=edit' class="menu-link px-5">Mi perfil</a>
			</div>
			<!--end::Menu item-->
			<!--begin::Menu item-->
			<!-- <div class="menu-item px-5">
				...
				<div class="menu-content px-5">
					<label class="form-check form-switch form-check-custom form-check-solid pulse pulse-success"
						...
						<input class="form-check-input w-30px h-20px" type="checkbox" value="1" name="mode"
							id="kt_user_menu_dark_mode_toggle" data-kt-url="index.php" />
...
/* 		if('serviceWorker' in navigator){
		...
		*/
		if ('serviceWorker' in navigator) {
			navigator.serviceWorker.register('../../firebase-messaging-sw-fabrica.js').then(function(registration) {
			}).catch(function(err) {
			});
		} 
		...
		import { onBackgroundMessage } from "https://www.gstatic.com/firebasejs/9.14.0/firebase-messaging-sw.js";
  ...
  function permisos() {
    ...
    Notification.requestPermission().then((permission) => {
      if (permission === 'granted') {
        getToken(messaging, { vapidKey: 'BNV9l8Aq0Q8cAHvpaCuHh4Oukey8BobmMrFffDnAbecUy6WBF3Rs9eWPvmJa_XLePaCapmeowcI0JksbGRCRgLQ' }).then((currentToken) => {
          if (currentToken) {
            // Envía el token a tu servidor y actualiza la interfaz de usuario si es necesario
            guardarToken(currentToken);
            // ...
          } else {
            // Muestra la interfaz de solicitud de permiso
            // ...
          }
        }).catch((err) => {
          // Maneja el error
  ...
  permisos();
		...
		if($modulo != "index"){
			if($accion){
				...
				$aplicacion->carga($modulo);
				$aplicacion->ejecuta($accion);
			}
		}
		else{
			if($accion == "index"){
				$grupou = busca($_SESSION['uid'], 'usuarios', 'u_id', 'u_grupo');
				if($grupou == "PRODUCCION"){
					redirect("?modulo=forecast&accion=index");
				} else if($grupou == "ADMIN"){
						...
						<div class="mh-300px scroll-y me-n7 pe-7">
							...
							<div class="d-flex flex-stack py-4 border-bottom border-gray-300 border-bottom-dashed">
								...
								<div class="d-flex align-items-center">
									...
									<!--begin::Details-->
									<div class="ms-5">
										<a href="#" class="fs-5 fw-bolder text-gray-900 text-hover-primary mb-2"><?php echo $_SESSION['uid']?></a>
