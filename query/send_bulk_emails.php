...
// 5) SMTP (desde la tabla usuarios)
$sql = 'SELECT u_mailcorp, u_contraseñacorp, u_host, u_puerto, u_remitente, u_seguridad
        FROM usuarios WHERE u_id = "'.$uuid.'" LIMIT 1';
$result = setq($sql);
if (!$result || $result->num_rows === 0) {
  echo json_encode(['ok'=>false,'msg'=>'Config SMTP no encontrada para el usuario']); exit;
}
list($coemail, $adminpassword, $hostcorp, $puerto, $remitente, $seguridad) = $result->fetch_row();
// Decodifica la contraseña (DEBE ser una contraseña de aplicación de Gmail)
$password = base64_decode($adminpassword);
//die("Contraseña: ".$password);

// Normaliza host/puerto/seguridad por si en BD vienen con "tls://...:587"
$host   = parse_url($hostcorp, PHP_URL_HOST) ?: preg_replace('~^(\w+://)~','',$hostcorp);
$port   = (int)(parse_url($hostcorp, PHP_URL_PORT) ?: $puerto ?: 587);
$secure = strtolower($seguridad);
if ($secure !== 'ssl' && $secure !== 'tls') $secure = 'tls';
...
$mail->SMTPSecure = $secure;     // 'tls' para 587, 'ssl' para 465
$mail->CharSet    = 'UTF-8';
...
$mail->Username   = $coemail;    // tu cuenta Gmail (ej. diegoyave22@gmail.com)
$mail->Password   = $password;   // APP PASSWORD de 16 chars
// Usa el nombre de la BD si existe, si no, 'DVLogistics'
$fromName = $remitente ?: 'DVLogistics';
$mail->setFrom($coemail, $fromName);
$mail->addReplyTo($coemail, $fromName);
$mail->SMTPDebug = 2;
$mail->Debugoutput = 'error_log';

// (opcional) debug temporal:
// $mail->SMTPDebug = 2;           // 0 en producción
// $mail->Debugoutput = 'error_log';

  /*
  $smtpHost = 'dvlogistics.mx';
  $smtpUser = 'no-reply@dvlogistics.mx';
  $smtpPass = '@dvl0gistic511075';
  $useSSL   = true;              // true = 465/ssl, false = 587/tls

  $mail = new PHPMailer(true);   // v5: clase global sin namespace
  $mail->isSMTP();
  $mail->Host       = $smtpHost;
  $mail->SMTPAuth   = true;
  $mail->Username   = $smtpUser;
  $mail->Password   = $smtpPass;
  $mail->Port       = $useSSL ? 465 : 587;
  $mail->SMTPSecure = $useSSL ? 'ssl' : 'tls';   // <-- v5 usa strings, NO constantes
  $mail->CharSet    = 'UTF-8';
  $mail->setFrom($smtpUser, 'DVLogistics');
  $mail->isHTML(true);
