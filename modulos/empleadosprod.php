...
/* ini_set('display_errors', 1); */
  class usuariosprod{
    ...
    function __construct(){
      $this->model = new modelusuariosprod(isset($obj));
    }
    ...
    function index(){
      ...
      $this->view = new viewusuariosprod($this->model);
    ...
    function edit(){
      ...
      $sqltotusu = 'SELECT COUNT(*) FROM usuariosprod WHERE u_empresa = "'.$_SESSION['emp'].'"';
      ...
      if($usuarios >= $licenciausu && $_GET['id'] == NULL){
        ...
        <script>
            ...
            window.location.href = "?modulo=usuariosprod&accion=index";
        </script>
        ';
      }else {
    ...
    function setusuario(){
      ...
      $countusu = busca($_SESSION['emp'],'usuariosprod','u_estatus = "A" AND u_empresa','COUNT(*)');
      ...
      if(isset($_POST['nuser'])) $nuser = $_POST['nuser']; else $nuser = getmax("u_nuser","usuariosprod");
        ...
        if($tamano_archivo > 3000000) $error = "6XMUE2";
          ...
          if (!((strpos($tipo_archivo, "gif") || strpos($tipo_archivo, "jpeg") || strpos($tipo_archivo, "jpg") || strpos($tipo_archivo, "png")))) {
            ...
            if (move_uploaded_file($_FILES['avatar']['tmp_name'],  'images/avatares/'.$nombre_archivo)){
              $sql = 'UPDATE usuariosprod SET u_avatar = "'.$nombre_archivo.'" WHERE u_id = "'.$_POST['id'].'"';
              setq($sql);
              $error="1";
            }
            else $error = "6XMUE4";
      ...
      redirect("?modulo=usuariosprod&accion=edit&id=".$this->model->id.'&error='.$okpass.'&avatar='.$error);
    }
    function delavatar(){
      unlink('images/avatares/'.busca($_GET['id'],'usuariosprod','u_id','u_avatar'));

      $sql = 'UPDATE usuariosprod SET u_avatar = NULL WHERE u_id = "'.$_GET['id'].'"';
      setq($sql);

      redirect("?modulo=usuariosprod&accion=edit&id=".$_GET['id']);
    }
    ...
    function setorden(){
      ...
      redirect("?modulo=usuariosprod&accion=show");
  ...
  class modelusuariosprod{
    
    function select($id){
      $sql = "SELECT * FROM usuariosprod WHERe u_id = '".$id."' AND u_empresa = '".$_SESSION['emp']."'";
    ...
    function result($page,$nmb,$grupo,$estatus){
      $pagenum = 50;
      $sql = 'SELECT * FROM usuariosprod ORDER BY up_id ASC';
    ...
    function setusuario(){
      $sql = "INSERT INTO usuariosprod SET
              u_nuser = '".$this->nuser."',
              u_id = '".$this->id."',
      ...
      if(!empty($this->passcorp)){
        $sql = "UPDATE usuariosprod SET u_passcorp= '".$this->passcorp."' WHERE u_nuser = '".$this->nuser."'";
        setq($sql);
      }
    }

    function setusuariosorden(){
      $sqls = 'SELECT u_grupo, u_estatus FROM usuariosprod WHERE u_id = "'.$this->id.'"';
      $results = setq($sqls);
    ...
    function setpass($id,$pass,$pass2){
      ...
      elseif($pass != $pass2) $error = "6XMU01";
        ...
        $sql = "UPDATE usuariosprod SET u_password = PASSWORD('".strtoupper($pass)."') WHERE u_id = '".$id."'";
        $error = setq($sql);
      }
...
class viewusuariosprod{
  ...
  function browse($page,$nmb,$grupo,$estatus){
    ...
    $nuevo = '<a id="nuevo" href="?modulo=usuariosprod&accion=edit" accesskey="">
            <button type="button" class="btn-sm btn btn-primary"><i class="fa fa-plus"></i> Nuevo </button>
          </a>';

    $orden = '
      <a href="?modulo=usuariosprod&accion=show">
    ...
    toolbar($_GET['modulo'],$nuevo,$filtro,$orden);
              ...
              <table class="mb-0 table table-hover" id="myTable">
                ...
                while($row = $this->model->result->fetch_array()){
                  echo '<tr>
                    ...
                    <th><a href="?modulo=usuariosprod&accion=edit&id='.$row['u_id'].'"><button type="button" class="btn btn-sm btn-secondary" /><i class="fas fa-edit"></i> Editar</button></a></th>
                  </tr>';
                }
              echo '</table>';
              /*
  ...
  function edit(){
    ...
    $atras = '
    <a href="?modulo=usuariosprod&accion=index">
    ...
    <div class="row mt-3">
      <form name="setuser" id="setuser" autocomplete="off" class="form row" method="post" action="?modulo=usuariosprod&accion=setusuario" enctype="multipart/form-data" onsubmit="checkguardar();">
        <?php
        if($this->model->nuser){
            ...
            <div class="card-block">
              <div class="form-body">
                  ...
                  <div class="col-md-6 col-sm-6 col-xs-12">
                    <div class="mb-5">
                      ...
                      <select class="mb-5 form-select form-select-solid" onchange="setNotify();" name="grupo" id="grupo" required="required" >
                        ...
                        <?php
                          $sql = 'SELECT * FROM grupos INNER JOIN grupos_empresas ON g_id = ge_grupo 
                                  WHERE g_estatus = "A" AND ge_estatus = "A" AND ge_empresa = "'.$_SESSION['emp'].'" AND g_id != "SUPER"';
                          $result = setq($sql);
                ...
                <div>
                  ...
                  <input type="checkbox" name="estatus" id="estatus" <?php echo $ch; ?> data-toggle="toggle" onchange="desactivarVendedor()" data-on="Activo" data-off="Inactivo" data-onstyle="success" data-offstyle="danger"/>
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              <div class="mb-5">
                <?php if(file_exists('images/avatares/'.$this->model->avatar) && $this->model->avatar){ ?>
                  <a href="?modulo=usuariosprod&accion=delavatar&id=<?php echo $this->model->id ?>">
    ...
    <script>
      ...
      function desactivarVendedor(){
        ...
        if(!estatus.checked){
          Swal.fire({
            title: 'Atención',
            text: "¿Estás seguro de marcar como descativado a este vendedor?. Todos sus leads serán reasignados a otros vendedores.",
            icon: 'warning',
  ...
  function show(){
    echo '
      ...
      function initEvents() {
          ...
          <div class="listWrap">
            <h1>Orden de los vendedores</h1>
              <table id="mi-tabla" class="list">
                  ...
                  $sqluo = "SELECT * FROM usuarios_orden INNER JOIN usuariosprod ON uo_uid = u_id WHERE u_estatus= 'A' AND u_grupo = 'VENTAS' ORDER BY uo_orden ASC";
                  $result = setq($sqluo);
        ...
        myForm.action = "?modulo=usuariosprod&accion=setorden";
