...
ini_set('display_errors', 1);
  class usuarioconfig{
    ...
    function __construct(){
      $this->model = new modelusuarioconfig(isset($obj));
    }

    function edit(){
        $this->model->select($_SESSION['uid']);
        $this->view = new viewusuarioconfig($this->model);
        $this->view->edit();
    }
    ...
    function setusuario(){
      ...
      if($countusu == $usulicencia && isset($_POST['estatus'])) {
        echo '<script>
          ...
          window.location.href = "?modulo=usuarioconfig&accion=index";
        </script>';
        die();
      } */
      ...
      if (!mb_check_encoding($saludo, 'UTF-8')) {
          $saludo = mb_convert_encoding($saludo, 'UTF-8');
      }

      // Ahora $emojiString está en formato UTF-8 y puedes usarlo en tu consulta SQL.
      $this->model->setdata($nuser,$_POST['id'],$_POST['nmb'],$_POST['apellidos'],$_POST['correo'],$_POST['telefono'],$_POST['puesto'],$_POST['nacimiento'],$_POST['grupo'],$_POST['comment'],'A',$_POST['mailcorp'],$_POST['passcorp'],$_POST['host'],$_POST['port'],$_POST['seguridad'],$_POST['correopas'],$_POST['remitente'],$_POST['color'], $notificaciones,$saludo);
      $this->model->setusuario();
      /* $this->model->setusuarioconfigorden(); */
      ...
      redirect("?modulo=usuarioconfig&accion=edit&id=".$this->model->id.'&error='.$okpass.'&avatar='.$error);
    }
    function delavatar(){
      ...
      redirect("?modulo=usuarioconfig&accion=edit&id=".$_GET['id']);
    }
    ...
    function setorden(){
      ...
      redirect("?modulo=usuarioconfig&accion=show");
  ...
  class modelusuarioconfig{
    
    function select($uid){
      $sql = 'SELECT * FROM usuarios WHERe u_id = "'.$uid.'" AND u_empresa = "'.$_SESSION['emp'].'"';
    ...
    function setdata($nuser,$id,$nmb,$apellidos,$correo,$telefono,$puesto,$nacimiento,$grupo,$comment,$estatus,$mailcorp,$passcorp,$host,$puerto,$seguridad,$correopass,$remitente,$color,$notificaciones,$saludo){
      ...
      $simbol = array('"', "'");
      $simbolcambio = array('\"', "\'");
      $saludo = str_replace($simbol, $simbolcambio, trim($saludo));
    ...
    function setusuarioconfigorden(){
      ...
      if($grupo == "VENTAS" && $estatus == "A"){
        ...
        $sql = 'INSERT IGNORE INTO usuarios_orden (uo_uid, uo_orden)
          VALUES ("' . $this->id . '", "' . $orden . '")';
        setq($sql);
...
class viewusuarioconfig{
  ...
  function browse($page,$nmb,$grupo,$estatus){
    ...
    $nuevo = '<a id="nuevo" href="?modulo=usuarioconfig&accion=edit" accesskey="">
            <button type="button" class="btn-sm btn btn-primary"><i class="fa fa-plus"></i> Nuevo </button>
          </a>';

    $orden = '
      <a href="?modulo=usuarioconfig&accion=show">
              ...
              <table class="mb-0 table table-hover" id="myTable">
                ...
                while($row = $this->model->result->fetch_array()){
                  echo '<tr>
                    ...
                    <th><a href="?modulo=usuarioconfig&accion=edit&id='.$row['u_id'].'"><button type="button" class="btn btn-sm btn-secondary" /><i class="fas fa-edit"></i> Editar</button></a></th>
                  </tr>';
                }
              echo '</table>';
            echo '</center>
  ...
  function edit(){
    ...
    $atras = '
    <a href="?modulo=usuarioconfig&accion=index">
    ...
    <meta charset="UTF-8">
    <div class="row mt-3">
      <form name="setuser" id="setuser" autocomplete="off" class="form row" method="post" action="?modulo=usuarioconfig&accion=setusuario" enctype="multipart/form-data" onsubmit="checkguardar();">
        <?php
        if($this->model->nuser){
            ...
            <div class="card-block">
              <div class="form-body">
                ...
                <div>
                  ...
                  <input type="checkbox" name="estatus" id="estatus" <?php /* echo $ch; */ ?> data-toggle="toggle" data-on="Activo" data-off="Inactivo" data-onstyle="success" data-offstyle="danger"/>
                ...
                <div class="row">
                  <div class="col-md-12">
                    <div class="mb-5">
                      <textarea name="saludo" rows="5" class="form-control" id="saludo"><?php echo $this->model->saludo; ?></textarea>
                    </div>
                  </div>
                </div>
                <?php
...
<script> 
 ...
 tinymce.init({
    ...
    setup: function (editor) {
                        ...
                        <div class="modal-content">
                          ...
                          <div class="modal-body">
                            <div class="mb-1" style="font-size: small;">
                                El correo empresarial funciona para realizar envios por correo electronico de manera automatica en los modulos de cotizaciones, remisiones y ordenes de compra del sistema.
              ...
              <div class="mb-5">
                        <label>Proveedor de Correo <a data-bs-toggle="modal" data-bs-target="#exampleModalAYUDA"><i class="fas fa-question-circle"></i></a></label>
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              <div class="mb-5">
                <?php if(file_exists('images/avatares/'.$this->model->avatar) && $this->model->avatar){ ?>
                  <a href="?modulo=usuarioconfig&accion=delavatar&id=<?php echo $this->model->id ?>">
  ...
  function show(){
    echo '
      ...
      function initEvents() {
        ...
        myForm.action = "?modulo=usuarioconfig&accion=setorden";
