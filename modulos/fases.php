...
function index(){    
    ...
    $this->view = new viewlfases($this->model);
...
function insert(){
  ...
  $veri = busca($_POST['nmb'], 'pr_fases', 'pf_planta = "'.$_POST['planta'].'" AND pf_nmb', 'COUNT(*)');
  if($veri > 0){
    echo '<script>alert("El nombre de la fase ya se encuentra asignada a la planta en el sistema, verifique su información.")</script>';
  }else {
    $this->model->setdata('',$_POST['nmb'], $_POST['descripcion'],$est,$_POST['planta']);
    $this->model->insert();
  }

  redirect('?modulo=fases&accion=index');
}
function update(){
  if(isset($_POST['estatus'])){
    $est = "A";
  ...
  $veri = busca($_POST['nmb'], 'pr_fases', 'pf_id != "'.$_POST['id'].'" AND pf_planta = "'.$_POST['planta'].'" AND pf_nmb', 'COUNT(*)');
  if($veri > 0){
    echo '<script>alert("El nombre de la fase ya se encuentra asignada a la planta en el sistema, verifique su información.")</script>';
  } else {
    $this->model->setdata($_POST['id'], $_POST['nmb'], $_POST['descripcion'],$est,$_POST['planta']);
    $this->model->update();
  }
...
function result($nmb,$estatus){ //filtro
  $sql = 'SELECT * FROM pr_fases WHERE 1=1 ';
  if($nmb) $sql.= ' AND pf_nmb LIKE "%'.trim($nmb).'%"';
  if($estatus) $sql.= ' AND pf_estatus="'.$estatus.'"';
  $sql.=' ORDER BY pf_id ASC ';
  $this->result = setq($sql);
  $this->resultt = setq($sql);
}
function select($id){
    $sql = 'SELECT * FROM pr_fases WHERE pf_id="'.$id.'"';
    $result = setq($sql);
    $row = $result->fetch_array();
    $this->id = $row['pf_id'];
    $this->nmb = $row['pf_nmb'];
    $this->descripcion = $row['pf_descripcion'];
    $this->estatus = $row['pf_estatus'];
}


function setdata($id,$nmb, $descripcion, $estatus,$planta){
...
function insert(){
    $sql = 'INSERT INTO pr_fases SET
            pf_nmb = "'.$this->nmb.'",
            pf_descripcion = "'.$this->descripcion.'",
            pf_estatus = "'.$this->estatus.'",
            pf_planta = "'.$this->planta.'"
            ';
    setq($sql);
}
function update(){
    $sql = 'UPDATE pr_fases SET
            pf_nmb = "'.$this->nmb.'",
            pf_descripcion = "'.$this->descripcion.'",
            pf_estatus = "'.$this->estatus.'",
            pf_planta = "'.$this->planta.'"
...
function browse($nmb,$estatus) {
   //Sección: E1 Encabezado - Botones de acción
    ...
    <?php
      ...
      $nuevo = '<a id="nuevo" class="btn btn-sm btn-primary" data-fancybox data-type="ajax" data-src="popup/setfase.php" href="javascript:;">
    ...
    $filtrar = ' <form class="form-inline" role="form" method="post" action="?modulo=fases&accion=index" id="filtro">
                ...
                <a href="?modulo=fases&accion=index"><button type="button" class="btn btn-warning">
      ...
      echo '<script>
            function checerase(iduni){
              var conf = confirm("¿Deseas eliminar La fases seleccionada?\nPresiona OK para borrarlo");
              if(conf == true){
                window.location.href="?modulo=fases&accion=borrar&id=" + iduni;
              }
            }
          </script>
  ...
  echo '
      ...
      while ($row = $this->model->result->fetch_array()) {
        ...
        echo '<tr>
        <td>'.$row['pf_nmb'].'</td>
        <td>'.$row['pf_descripcion'].'</td>
        <td>'.busca($row['pf_planta'],'pr_plantas','pp_id','pp_nmb').'</td>
        ...
        /*
          ...
          echo '<a data-toggle="tooltip" data-placement="top" title data-original-title="Editar la fases" data-fancybox data-type="ajax" data-src="popup/setfase.php?id='.$row['pf_id'].'" href="javascript:;">
          ...
          /* echo '<button type="button" class="btn btn-danger" onclick="checerase('.$row['pf_id'].')">
