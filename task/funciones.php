...
function existenciaxestatus($almacen, $estatus){
  ...
  if($estatus == "LA"){ //Libre-apartado
    $sqlla = 'SELECT * FROM crm_cotizaciones INNER JOIN crm_tableros ON ct_id = cc_tablero 
                        WHERE ct_nnegociacion = "2" AND cc_estatus IN ("A", "V") AND cc_almacen = "'.$almacen.'"';

  } else if($estatus == "VA"){ //Vendido-abonado
    $sqlla = 'SELECT * FROM crm_cotizaciones INNER JOIN remisiones ON r_id = cc_remision INNER JOIN cxcobrar ON cx_referencia = r_id 
                        WHERE r_estatus = "A" AND cx_estatus IN ("N", "A") AND r_almacen = "'.$almacen.'"';

  } else if($estatus == "VP"){ //Vendido-pagado
    $sqlla = 'SELECT * FROM crm_cotizaciones INNER JOIN remisiones ON r_id = cc_remision INNER JOIN cxcobrar ON cx_referencia = r_id
                WHERE r_estatus = "F" AND cx_estatus = "F" AND r_almacen = "'.$almacen.'"';
  }

  $resultla = setq($sqlla);
  //echo $sqlla.'<br>';
  while($row = $resultla -> fetch_array()){
    $sqldet = 'SELECT * FROM crm_cotizacionesd INNER JOIN articulos ON a_id = cdm_articulo WHERE cdm_cotizacion = "'.$row['cc_id'].'"';
    $resultdet = setq($sqldet);
    while($rowdet = $resultdet -> fetch_array()){
      if($rowdet['a_tipoprod'] == "M"){
        $sqlcmb = 'SELECT * FROM crm_cotizacion_variantes WHERE ccv_cotizacion = "'.$row['cc_id'].'" AND ccv_cotizaciond = "'.$rowdet['cdm_id'].'"';
        $resultcmb = setq($sqlcmb);
        //echo $sqlcmb.'<br>';
        while($rowcmb = $resultcmb -> fetch_array()){
          if(!isset($datos[$rowcmb['ccv_articulo']][$rowcmb['ccv_modelo']])) $datos[$rowcmb['ccv_articulo']][$rowcmb['ccv_modelo']] = 0;
          $datos[$rowcmb['ccv_articulo']][$rowcmb['ccv_modelo']] += floatval(($rowcmb['ccv_cantidad'] * $rowdet['cdm_cantidad']));
          $sqllig = 'SELECT * FROM articulos_ligados WHERE al_articulo = "'.$rowcmb['ccv_articulo'].'"';
          $resultlig = setq($sqllig);
          while($rowlig = $resultlig -> fetch_array()){
            if(!isset($datos[$rowlig['al_aligado']][$rowlig['al_amodelo']])) $datos[$rowlig['al_aligado']][$rowlig['al_amodelo']] = 0;
            $datos[$rowlig['al_aligado']][$rowlig['al_amodelo']] += floatval(($rowlig['al_cantidad']*$rowdet['cdm_cantidad']));
          }
          //echo 'articulo: '.$rowcmb['ccv_articulo'].' modelo: '.$rowcmb['ccv_modelo'].' agregado: '.floatval(($rowcmb['ccv_cantidad'] * $rowdet['cdm_cantidad'])).'<br>';
        } 

      } else {
        if(!isset($datos[$rowdet['cdm_articulo']][$rowdet['cdm_modelo']])) $datos[$rowdet['cdm_articulo']][$rowdet['cdm_modelo']] = 0;
        $datos[$rowdet['cdm_articulo']][$rowdet['cdm_modelo']] += floatval($rowdet['cdm_cantidad']);
        $sqllig = 'SELECT * FROM articulos_ligados WHERE al_articulo = "'.$rowdet['cdm_articulo'].'"';
        $resultlig = setq($sqllig);
        while($rowlig = $resultlig -> fetch_array()){
          if(!isset($datos[$rowlig['al_aligado']][$rowlig['al_amodelo']])) $datos[$rowlig['al_aligado']][$rowlig['al_amodelo']] = 0;
          $datos[$rowlig['al_aligado']][$rowlig['al_amodelo']] += floatval(($rowlig['al_cantidad']*$rowdet['cdm_cantidad']));
        }
        //echo 'articulo: '.$rowdet['cdm_articulo'].' modelo: '.$rowdet['cdm_modelo'].' agregado: '.floatval($rowdet['cdm_cantidad']).'<br>';
      }
...
function validaestatus($accion){
  if($accion == "corte"){
    ...
    while($row = $result -> fetch_array()){
      $remision = $row['r_id'];
      $motivo = 'Se superó la fecha límite de pago';
...
{ //Realizar una consulta a BD en primer nivel
  $dbuser = "inflalandia_root"; // El usuario
  $dbpass = "Infla2023#"; // El Pass

  // $dbuser = "root"; // El usuario
  // $dbpass = ""; // El Pass

  $dbhost = "162.241.92.192"; // El host
  ...
  if ($modo == "S") {
    ...
    while ($row2 = $result->fetch_array()) {
      if ($row2['e_cantidad'] > ($cantidad - $contados)) {
        ...
        setq($sql) or die($sql);
        $contados += $row2['e_cantidad'] - ($cantidad - $contados);
        break;
      } else {
    ...
    if($restante > 0){
      ...
      if($xpid){
        ...
        $sqlres = 'UPDATE existencia_pendiente SET xp_cantidad = "'.$cantnew.'" WHERE xp_id = "'.$xpid.'"';
      } else{
      ...
      setq($sqlres);    
      if($tipomod == "R"){
        $sqlfal = 'SELECT rf_id, rf_cantidad FROM remision_faltantes WHERE rf_articulo = "'.$articulo.'" AND rf_modelo = "'.$modelo.'" AND rf_estatus = "N" AND rf_remision = "'.$idmov.'"';
        $resulfal = setq($sqlfal);
        list($idfal, $canfal) = $resulfal -> fetch_array();
        if($idfal){
          $cantnue = $restante + $canfal;
          $sqlrem = 'UPDATE remision_faltantes SET rf_cantidad = "'.$cantnue.'" WHERE rf_id = "'.$idfal.'"';
        } else {
          $sqlrem = 'INSERT INTO remision_faltantes SET rf_articulo = "'.$articulo.'",
                                                      rf_modelo = "'.$modelo.'",
                                                      rf_cantidad = "'.$restante.'",
                                                      rf_estatus = "N",
                                                      rf_remision = "'.$idmov.'"
                                                      ';
        }
    ...
    if($idrep){
      ...
      if($rest > 0){
        ...
        $cantidad = 0;
        if($idmov){
          $sqlupd = 'UPDATE movimientosd SET md_repuestos = "'.$rest.'" WHERE md_movimiento = "'.$idmov.'" AND md_articulo = "'.$articulo.'" AND md_modelo = "'.$modelo.'"';
          setq($sqlupd);
        }
      } else if($rest == 0) {
...
function reasignarLeads(){
  $numeroDiaSemana = intval(date('N')); //Obtenemos el día de la semana

  if ($numeroDiaSemana != 7) {
    $diasSemana = array(1, 2, 3, 4, 5, 6); //El númeto de los dias en los que se trabaja

    $restarDias = 5; //El número de días que se restará
    $diasTotal = intval(count($diasSemana));
    if ($numeroDiaSemana > $restarDias) { //Si el número de días a restar el mayor que al número del día de hoy obtenemos la diferencia
      $diferencia = $numeroDiaSemana - $restarDias;
      $numeroDia = $diasSemana[($diferencia - 1)];
    } else {
      $diferencia = $restarDias - $numeroDiaSemana;
      $dif = $diasTotal - $diferencia;
      $numeroDia = $diasSemana[($dif - 1)];
    }

    // Convierte el número del día en un formato de día de la semana
    $nombreDia = date('l', strtotime("Sunday +$numeroDia days"));

    $hoy = new DateTime(); // Fecha actual

    // Calcula cuántos días debes retroceder para llegar al último día de la semana correspondiente
    $diasParaRetroceder = ($hoy->format('N') - $numeroDia + 7) % 7;

    // Resta la cantidad de días necesarios para llegar al último día de la semana correspondiente
    $fechaUltimoDia = $hoy->sub(new DateInterval("P{$diasParaRetroceder}D"));

    // Formatea la fecha en 'Y-m-d'
    $fechaMenosTresDias = $fechaUltimoDia->format('Y-m-d');
    ...
    //Consultamos los leads que estan asignados o reasignados de tres dias atrás que no tengan un tablero abierto
    $sql = 'SELECT DISTINCT cl_id FROM crm_leads 
          WHERE cl_estatus IN ("A", "R") AND cl_fasigna = "' . $fechaMenosTresDias . '" AND cl_id 
            NOT IN ( SELECT DISTINCT cl_id FROM crm_leads INNER JOIN crm_clientes ON (c_telefono1 LIKE CONCAT("%", cl_telefono ,"%") OR c_telefono2 LIKE CONCAT("%", cl_telefono,"%")) 
                    INNER JOIN crm_tableros ON ct_cliente = c_id 
                    WHERE cl_estatus IN ("A", "R") AND cl_fasigna = "' . $fechaMenosTresDias . '") LIMIT 0,500';
    $result = setq($sql);
    ...
    while ($row = $result->fetch_array()) {
      ...
      $sqlh = 'SELECT hl_vendedor FROM historial_leadsasignacion INNER JOIN usuarios ON u_id = hl_vendedor WHERE u_estatus = "A" AND hl_lead = "' . $idlead . '"';
      ...
      if (count($diferencia) > 0) {
        ...
        */
        ...
        $sqlr = 'SELECT hl_vendedor, COUNT(*) AS cantidad
              FROM historial_leadsasignacion INNER JOIN usuarios ON u_id = hl_vendedor WHERE u_estatus = "A" AND hl_vendedor != "' . $ultimovendedor . '"
        ...
        if ($filas > 1) {
          ...
          $sqlt = 'SELECT hl_vendedor FROM historial_leadsasignacion INNER JOIN usuarios ON u_id = hl_vendedor WHERE u_estatus = "A" AND hl_lead = "' . $idlead . '" ORDER BY hl_fasigna ASC LIMIT 1';
          $resultt = setq($sqlt);
